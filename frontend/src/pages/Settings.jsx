...
export default function Settings() {
  ...
  const handlePassword = async (e) => {
    ...
    if (!pw.securityAnswer.trim()) {
      addToast('Enter the answer to your security question.');
      return;
    }
    setPwSaving(true);
    try {
      ...
      setPw({ securityAnswer: '', newPassword: '', confirm: '' });
    } catch (err) {
      ...
      <div className="max-w-2xl mx-auto px-5 py-8 lg:py-12 space-y-8">
        ...
        <section className="p-7 sm:p-8" style={{ ...GLASS, ...CARD_RADIUS }}>
          ...
          <p className="text-xs text-dark-ash mb-6 leading-relaxed">
            Your saved question is shown below. Enter the matching answer in capital letters, then choose a new password.
          </p>
          <form onSubmit={handlePassword} className="space-y-4">
            <div>
              <label className="block text-sm text-dark-ash mb-2">Your security question</label>
              <div className="w-full px-4 py-3 rounded-xl bg-page/50 border border-ash/25 text-sm text-charcoal tracking-wide">
                {user?.securityQuestion || 'No security question on file. Complete onboarding first.'}
              </div>
              <p className="text-[11px] text-dark-ash mt-1.5">Saved at registration — answer only below.</p>
            </div>
            <div>
              <label className="block text-sm text-dark-ash mb-2">Your answer</label>
              <input
                value={pw.securityAnswer}
                onChange={(e) => setPw({ ...pw, securityAnswer: e.target.value.toUpperCase() })}
                placeholder="ANSWER IN CAPITAL LETTERS"
