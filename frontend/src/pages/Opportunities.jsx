      ...
      <div className="max-w-5xl mx-auto px-5 py-10">
        ...
        {loading ? (
          ...
          <GlassCard className="p-12 text-center">
            <h3 className="font-heading font-semibold text-lg mb-3">No official opportunities cached</h3>
            <p className="text-charcoal text-sm max-w-md mx-auto mb-6 leading-relaxed">
              {meta?.note ||
                'Opportunity data comes only from Zero Authority DAO (primary) and official Stacks APIs (secondary). Configure credentials in the backend environment and run a sync. Nothing is ever fabricated.'}
            </p>
            <button
              onClick={handleSync}
              disabled={syncing}
          ...
          <div className="space-y-4">
            {items.map((o) => (
              <GlassCard key={o._id} className="p-6">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs uppercase tracking-wide text-ash">{o.type}</span>
                      <span className="text-xs text-ash">· {o.source?.replace(/_/g, ' ')}</span>
                    </div>
                    <h3 className="font-heading font-semibold text-lg">{o.title}</h3>
                    {o.organizer && <p className="text-sm text-ash mt-1">{o.organizer}</p>}
                    {o.description && (
                      <p className="text-sm text-charcoal mt-3 line-clamp-3">{o.description}</p>
                    )}
                  </div>
                  <div className="text-xs text-ash shrink-0">
        ...
        <p className="text-xs text-ash mt-10">
