  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-dark-ash text-sm mt-1">Chronological record for {project.name}.</p>
      </div>
          ...
          {events.map((ev) => (
            <GlassCard key={ev._id} className="p-5 flex gap-4 items-start">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-dark-ash mt-1">{new Date(ev.createdAt).toLocaleString()}</p>
              </div>
            </GlassCard>
          ))}
        </div>
