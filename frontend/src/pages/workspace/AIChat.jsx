...
function AssistantMessage({ content, animate }) {
  const safe = typeof content === 'string' ? content : textOf({ content });
  const [copied, setCopied] = useState(false);
  const [collapsed, setCollapsed] = useState(() => safe.length > COLLAPSE_CHARS);
  const isLong = safe.length > COLLAPSE_CHARS;
  ...
  const shown = collapsed && isLong ? `${safe.slice(0, COLLAPSE_CHARS).trim()}…` : safe;

  return (
    <div className={animate ? 'ai-reveal' : ''}>
      <div className="rounded-2xl border border-ash/20 bg-white/80 px-5 py-5 shadow-sm min-w-0">
...
export default function AIChat() {
  ...
  const handleSend = async (e) => {
    ...
    try {
      // Always pass plain string content — never an object
      const data = await sendMessage(projectId, text, mode);
      const incoming = data.messages || [];
      if (incoming.length) {
        // Append to history (server returns only the new pair)
        setMessages((prev) => {
          const withoutTmp = prev.filter((m) => m._id !== optimistic._id);
          // Avoid dupes if server echoed full history
          if (incoming.length > 2) return incoming;
          return [...withoutTmp, ...incoming];
        });
        const lastAsst = [...incoming].reverse().find((m) => m.role === 'assistant');
        if (lastAsst) setLatestAssistantId(String(lastAsst._id || lastAsst.id));
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6 min-w-0">
      ...
      <GlassCard className="p-5 sm:p-6 min-h-[280px] flex flex-col min-w-0">
        <div className="flex-1 space-y-4 mb-4 max-h-[min(58vh,560px)] overflow-y-auto overflow-x-auto pr-1 min-w-0">
          {loading ? (
            <ThinkingSkeleton />
          ) : messages.length === 0 ? (
            ...
            messages.map((msg) =>
              msg.role === 'assistant' ? (
                <AssistantMessage
                  key={msg._id || msg.id}
                  content={textOf(msg)}
                  animate={String(msg._id || msg.id) === String(latestAssistantId)}
                />
              ) : (
                <div
                  key={msg._id || msg.id}
                  className="ml-auto max-w-[85%] rounded-2xl bg-black text-white px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap break-words"
                >
                  {textOf(msg)}
                </div>
              )
      ...
      <style>{`
        ...
        .ai-reveal { animation: ai-reveal 220ms ease-out; }
        @keyframes skel-pulse {
          0%, 100% { opacity: 0.35; }
          50% { opacity: 0.85; }
        }
        .ai-skel-bar { animation: skel-pulse 1.4s ease-in-out infinite; }
        .ai-skeleton { animation: ai-reveal 280ms ease-out; }
      `}</style>
    </div>
  );
}
