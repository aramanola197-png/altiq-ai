...
export default function ProjectWorkspace() {
  ...
  const onTabScroll = () => {
    ...
    scrollTimer.current = setTimeout(() => setTabScrolling(false), 700);
  };
    ...
    <AppShell>
      <div className="max-w-5xl mx-auto px-5 py-8">
        ...
        <div className="relative mb-8">
          {/* Edge fades — only while the tab strip is being scrolled */}
          <div
            className={`pointer-events-none absolute inset-y-0 left-0 w-8 z-10 bg-gradient-to-r from-page to-transparent transition-opacity duration-300 ${
              tabScrolling ? 'opacity-100' : 'opacity-0'
            }`}
            aria-hidden
          />
          <div
            className={`pointer-events-none absolute inset-y-0 right-0 w-8 z-10 bg-gradient-to-l from-page to-transparent transition-opacity duration-300 ${
              tabScrolling ? 'opacity-100' : 'opacity-0'
            }`}
            aria-hidden
          />
          ...
          <div
            ...
            className="flex gap-1 overflow-x-auto border-b border-ash/25 pb-px scrollbar-none"
            style={{
              scrollbarWidth: tabScrolling ? 'thin' : 'none',
              msOverflowStyle: tabScrolling ? 'auto' : 'none',
            }}
          >
            {tabs.map((tab) => (
              <NavLink
