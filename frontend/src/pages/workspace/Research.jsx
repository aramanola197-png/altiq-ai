...
import { getResearch, generateResearch, updateResearch, exportUrl } from '../../api/workspaceApi';
...
export default function Research() {
  ...
  const handleGenerate = async () => {
    ...
    const interval = setInterval(() => {
      setStatus((s) =>
        s === 'Analyzing market…' ? 'Mapping competitors…' : s === 'Mapping competitors…' ? 'Structuring report…' : 'Analyzing market…'
      );
    }, 2000);
    try {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-ash text-sm mt-1">Structured market and competitor analysis for {project.name}.</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <GlassCard className="p-12 text-center">
          <p className="text-charcoal text-sm mb-6">
            No research yet. Generate a structured report covering problem analysis, market overview, competitors, risks, and recommendations.
          </p>
          <button
            onClick={handleGenerate}
            disabled={generating}
        ...
        <GlassCard className="p-8">
          <div className="flex justify-between items-center mb-6 text-xs text-ash gap-3 flex-wrap">
            <span>Version {report.version}</span>
            <div className="flex gap-3 items-center">
              <span>{new Date(report.createdAt).toLocaleString()}</span>
              <a href={exportUrl(projectId, 'research', 'markdown')} className="underline underline-offset-2 hover:text-black" download>Export Markdown</a>
              <a href={exportUrl(projectId, 'research', 'json')} className="underline underline-offset-2 hover:text-black">Export JSON</a>
            </div>
          </div>
