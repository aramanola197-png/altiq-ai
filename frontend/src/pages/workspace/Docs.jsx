  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-ash text-sm mt-1">Professional documents for {project.name}. Editable history is versioned.</p>
      </div>
            ...
            {Object.values(latestByType).map((d) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 rounded-xl text-sm transition ${
                  active?._id === d._id ? 'bg-black/5 text-black' : 'text-ash hover:bg-black/5'
                }`}
              >
                <div className="font-heading font-medium capitalize">{d.type}</div>
                <div className="text-xs text-ash mt-0.5">v{d.version}</div>
              </button>
            ))}
          </div>
          {active && (
            <GlassCard className="p-8">
              <div className="flex justify-between items-center mb-6 text-xs text-ash">
              ...
              <div className="text-sm leading-relaxed whitespace-pre-wrap text-charcoal">
                {active.content}
              </div>
            </GlassCard>
          )}
