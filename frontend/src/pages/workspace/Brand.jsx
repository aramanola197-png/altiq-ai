...
export default function Brand() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-dark-ash text-sm mt-1">
            Positioning, voice, and brand guidance for {project.name}. Logo assets remain
            owner-supplied.
          </p>
        </div>
        <button
      ...
      {loading ? (
        <GlassCard className="p-8 space-y-3 min-w-0">
        ...
        <GlassCard className="p-12 text-center">
          <p className="text-charcoal text-sm mb-6">
            No brand guidance yet. Generate positioning, voice, and messaging for this project.
          </p>
          <button
            onClick={handleGenerate}
            disabled={generating}
        ...
        <GlassCard className="p-8 min-w-0 overflow-x-auto">
          ...
          <DownloadPdfButton projectId={projectId} kind="brand" label="Download Brand PDF" />
        </GlassCard>
      )}
    </div>
  );
