...
/**
 * Desktop chrome (sidebar) when:
 * - width >= 900 (includes most "Desktop site" browser modes), OR
 * - landscape with width >= 568 (phones turned sideways, tablets)
 * Uses innerWidth/innerHeight so rotation + Desktop-site mode both work.
 */
function useDesktopChrome() {
  ...
  useEffect(() => {
    const apply = () => {
      ...
      const h = window.innerHeight;
      const landscape = w > h;
      // Desktop-site on mobile often reports ~980–1200 width even in "portrait"
      setDesktop(w >= 900 || (landscape && w >= 568));
    };
    apply();
    window.addEventListener('resize', apply);
    window.addEventListener('orientationchange', apply);
    // iOS fires orientationchange before dimensions update
...
export default function AppShell({ children }) {
  ...
  return (
    <div className="min-h-screen bg-page text-black flex relative">
      ...
      <aside
        className={`${isDesktop ? 'flex' : 'hidden'} flex-col w-64 border-r border-ash/25 py-8 sticky top-0 h-screen z-10`}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <main className={`flex-1 overflow-x-hidden relative z-10 ${isDesktop ? 'pt-0' : 'pt-[4.75rem]'}`}>
